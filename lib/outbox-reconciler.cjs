...
function createOutboxReconciler({
  ...
  async function attemptSend(filename, data, isRetry) {
    ...
    // Group messages: treat a successful send (got message ID) as
    // delivered immediately. WhatsApp group delivery acks are unreliable
    // and retrying just spams every member with duplicate messages.
    if (data.chat_id && data.chat_id.endsWith("@g.us") && newIds.length > 0) {
      try { fs.unlinkSync(path.join(outboxDir, filename)); } catch {}
      unregister(filename);
      sendState.delete(filename);
      return;
