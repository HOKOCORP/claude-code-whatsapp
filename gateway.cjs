...
function syncUserUsage(userId) {
  ...
  if (newCost > 0) {
    ...
    // Daily charge accumulator — tracked per-day so we can issue a
    // soft warning when a single user spends more than the operator's
    // comfort threshold in 24h. Resets implicitly when the date key
    // rolls over.
    if (!usage.daily_charge) usage.daily_charge = {};
    usage.daily_charge[today] = (usage.daily_charge[today] || 0) + userCharge;
    ...
    usage.daily_charge[today] = Math.round(usage.daily_charge[today] * 10000) / 10000;
    // Trim daily_charge to last 14 days so the file doesn't grow.
    const days = Object.keys(usage.daily_charge).sort();
    while (days.length > 14) delete usage.daily_charge[days.shift()];

    // Soft daily warning: if today's total just crossed the threshold
    // AND we haven't warned today, flag for the caller to send a
    // one-time message. Operator-tunable via DAILY_SOFT_WARN_GBP env.
    const dailyWarnAt = Number(process.env.DAILY_SOFT_WARN_GBP) || 20;
    if (!usage.daily_warned) usage.daily_warned = {};
    if (usage.daily_charge[today] >= dailyWarnAt && !usage.daily_warned[today]) {
      usage.daily_warned[today] = true;
      dailyWarnPending = true;
      // Trim daily_warned in lockstep with daily_charge.
      for (const d of Object.keys(usage.daily_warned)) {
        if (!(d in usage.daily_charge)) delete usage.daily_warned[d];
      }
    }
  }
  ...
  // Stash the just-crossed-threshold flag on the returned object so
  // the caller (checkUserLimit) can surface it without re-querying.
  // Not persisted — it's a one-shot signal for this call only.
  if (dailyWarnPending) usage._dailyWarnPending = true;
  return usage;
}
...
function checkUserLimit(userId) {
  ...
  if (mode === "blocked") {
    return { allowed: false, balance, warned: false, blocked: true, mode, minBalance, isAdmin: false, reason: "blocked", dailyWarn: false, dailyTotal: 0 };
  }

  const allowed = balance >= minBalance;
  const today = todayKey();
  const dailyTotal = (usage.daily_charge && usage.daily_charge[today]) || 0;
  return {
    allowed,
    balance,
    ...
    dailyWarn: !!usage._dailyWarnPending,
    dailyTotal,
  };
}
...
async function handleInviteCommands({ sock, msg, jid }) {
  ...
  // chatbots, including any external-facing or interactive AI agent,
  // must disclose to users that they are interacting with AI rather
  // than a human." This command surfaces that disclosure plus the
  // bot's no-refund policy and any ToS / privacy URLs the operator
  // has configured. DM-only — group chats see the disclosure baked
  // into the TOS-acceptance prompt.
  if (lower === "/about") {
    if (jid.endsWith("@g.us")) {
      try { await sock.sendMessage(jid, { text: "🚫 `/about` is DM-only." }); } catch {}
      return true;
    }
    const tosUrl = process.env.BOT_TOS_URL || "";
    const privacyUrl = process.env.BOT_PRIVACY_URL || "";
    const lines = [
      "🤖 *About this bot*",
      "",
      "This is an AI assistant powered by *Claude* (Anthropic). Every reply is generated by the model, not a human. Conversations are processed by Anthropic per their privacy policy.",
      "",
      "*Pricing:*",
      "• Shared-key mode: 2× the underlying API cost in £ (1 GBP ≈ 1 USD).",
      "• BYOK mode (`/cckey <APIKEY>`): 1× service fee in £, you pay Anthropic direct.",
      "• Minimum balance to send a message: £10.",
      "",
      "*Refunds:* All sales final. Credits are non-refundable once purchased, whether used or unused.",
      "",
      "*Top up:* `/topup <amount>` (card) or `/redeem <CODE>` (top-up code).",
      "*Status:* `/balance`, `/usage`, `/receipts`.",
      "*Help:* `/help` for the full command list.",
    ];
    if (tosUrl) lines.push("", `Terms: ${tosUrl}`);
    if (privacyUrl) lines.push(`Privacy: ${privacyUrl}`);
    try { await sock.sendMessage(jid, { text: lines.join("\n") }); } catch {}
    return true;
  }

  // ── /balance — concise balance display (alias-style for /usage) ──
  if (lower === "/balance") {
    if (jid.endsWith("@g.us")) {
      try { await sock.sendMessage(jid, { text: "🚫 `/balance` is DM-only." }); } catch {}
      return true;
    }
    const adminCheck = loadAdmin();
    if (isAdminJid(jid, adminCheck)) {
      try { await sock.sendMessage(jid, { text: "👑 Admin — unmetered." }); } catch {}
      return true;
    }
    const u = syncUserUsage(sanitizeUserId(jid));
    const cfg = loadUsageConfig();
    const lines = [`💰 Balance: *${fmtGbp(u.balance || 0)}*`];
    lines.push(`Mode: ${u.mode || "api-shared"}`);
    if (u.balance < cfg.min_balance) {
      lines.push("");
      lines.push(`⚠️ Below minimum (${fmtGbp(cfg.min_balance)}) — top up to send messages.`);
      lines.push("`/topup <amount>` or `/redeem <CODE>`");
    } else if (u.balance <= cfg.warn_balance) {
      lines.push("");
      lines.push(`⚠️ Running low. Top up soon.`);
    }
    try { await sock.sendMessage(jid, { text: lines.join("\n") }); } catch {}
    return true;
  }
    ...
    for (const msg of messages) {
      ...
      if (_admin && !_isAdminSender) {
        ...
        if (_gate.warned) {
          ...
          try {
            await sock.sendMessage(jid, { text:
              `⚠️ Balance running low: ${fmtGbp(_gate.balance)}. Top up with \`/topup <amount>\` or \`/redeem <CODE>\` soon to avoid service interruption.`
            });
          } catch {}
        }
        if (_gate.dailyWarn) {
          // Soft daily-spend warning — fired once per day per user
          // when today's running charge crosses DAILY_SOFT_WARN_GBP.
          // Doesn't block; user keeps using the bot. Just a heads-up
          // in case a session is running away.
          const dailyWarnAt = Number(process.env.DAILY_SOFT_WARN_GBP) || 20;
          try {
            await sock.sendMessage(jid, { text:
              `⚠️ *Daily spend notice*\n\n`
              + `You've spent ${fmtGbp(_gate.dailyTotal)} so far today (threshold ${fmtGbp(dailyWarnAt)}). Keep going if that's expected, or use \`/clear\` to drop conversation context if you suspect a runaway session.`
            });
          } catch {}
          log(`daily-warn: ${formatJid(_senderJid)} crossed ${fmtGbp(dailyWarnAt)}/day (today ${fmtGbp(_gate.dailyTotal)})`);
        }
      }
